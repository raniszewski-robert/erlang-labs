...
add_station_test() ->
  ...
  ?assertNotMatch({error, _}, pollution:add_station("Stacja 2", {2, 2}, M2)).
...
get_area_mean_test() ->
  ...
  ?assertMatch(15.0, pollution:get_area_mean("PM10", {1, 1}, 1.5, M4)),
  ?assertMatch(10.0, pollution:get_area_mean("PM10",{1, 1}, 1.0, M4)),
  ?assertMatch(20.0, pollution:get_area_mean("PM10",{2, 2}, 1.0, M4)),

  M5 = pollution:add_value("Stacja 1", {{2023,3,27},{11,16,14}}, "PM25", 100, M4),
  M6 = pollution:add_value("Stacja 2", {{2023,3,27},{11,16,15}}, "PM25", 220, M5),

  ?assertMatch(15.0, pollution:get_area_mean("PM10", {1, 1}, 1.5, M6)),
  ?assertMatch(160.0, pollution:get_area_mean("PM25", {1, 1}, 1.5, M6)),

  M7 = pollution:add_value("Stacja 3", {{2023,3,27},{11,16,18}}, "PM10", 1234, M6),
  ?assertMatch(258.8, pollution:get_area_mean("PM10", {2, 2}, 1.5, M7)).
...
get_area_mean_fail_test() ->
  ...
  ?assertMatch({error, _}, pollution:get_area_mean("PM10", {1, 1}, 1.5, M)),

  M1 = pollution:add_value("Stacja 1", {{2023,3,27},{11,16,10}}, "PM10", 10, M),
  M2 = pollution:add_value("Stacja 2", {{2023,3,27},{11,16,11}}, "PM10", 20, M1),

  ?assertMatch({error, _}, pollution:get_area_mean("PM25", {3, 3}, 10, M2)),
  ?assertMatch({error, _}, pollution:get_area_mean("PM10",{100, 100}, 3, M2)).